...
#edu_item(
  ...
  [Concerntrating in _Embedded Devices_ & _Systems and Architecture_],
...
#exp_item(
  ...
  [Created two internal tools with _Rust_, _React.js_, and _TypeScript_ which increased productivity by reverse engineering proprietary solutions for our workflow],
  [Programmed a microcontroller, using _C_ and _Rust_, which uses Modbus to extract data from a monitoring device],
)
#exp_item(
  ...
  [ Architected and implemented full-stack web applications using _Node.js_, _React.js_, _Docker_, and _AWS_],
  [Created a web application for a trucking company using _React.js_, _Next.js_, _Nest.js_, and _Nginx_. It was used to store information for thousands of loads and was used to make over _\$1 million_ in revenue],
)
...
#project_item(
  ...
  [Solved flaws in an existing robot dashboard application while improving user experience and performance],
...
#project_item(
  ...
  [Used to collect data from matches at robotics competitions. Implements authentication and data analysis tools],
  [Enabled our team to make informed, data-driven decisions during competitions],
)
#project_item(
  name: "ESP Spotify Display",
  skills: "Embedded, Rust",
  [Created an embedded project that shows what I'm listening to on Spotify through an _AWS Lambda_ function],
